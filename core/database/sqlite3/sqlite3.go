...
const (
	default_history_table = "schema_history"
...
func (r *SQLiteRepository) CheckSchemaHistoryTable() (bool, error) {
	...
	name := ""
	err := r.queriable.QueryRowContext(r.ctx, query, r.history_table).Scan(&name)
	if err != nil {
...
func (r *SQLiteRepository) ExecuteMigration(migration *migrations.Migration) []error {
	...
	query := fmt.Sprintf(`
		...
		VALUES ($1, $2, $3, $4)
		ON CONFLICT (version)
		DO UPDATE SET description = $2, md5_checksum = $3, success = $4, executed_at = current_timestamp;
	`, r.history_table)
...
func (r *SQLiteRepository) lock() error {
	query := `
		SELECT name
		FROM sqlite_schema
		WHERE type='table' AND name=?;
	...
	for range 12 {
		name := ""
		err := r.db.QueryRowContext(r.ctx, query, lock_table).Scan(&name)
		if err != nil {
			if err != sql.ErrNoRows {
				return err
			}
...
func (r *SQLiteRepository) unlock() error {
